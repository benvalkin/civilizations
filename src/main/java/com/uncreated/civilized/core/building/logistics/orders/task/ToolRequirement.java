...
public class ToolRequirement extends TaskItemRequirement {
   ...
   public ToolRequirement(Level level, String taskName, Class<? extends Item> toolItemType, Origin origin) {
      super(level, taskName, i -> toolItemType.isInstance(i.getItem()), origin);
   }

   public ToolRequirement(
         Level level,
         String taskName,
         Class<? extends Item> toolItemType,
         HolderSet<Block> exampleRequiredMinableBlocks,
         Origin origin) {
      super(level, taskName, i -> {

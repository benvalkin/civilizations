...
public abstract class TaskItemRequirement extends LogisticsOrder {
    ...
    public TaskItemRequirement(String taskName, Predicate<ItemStack> itemSearch, Origin origin) {
        super(taskName, itemSearch, origin);
        this.taskName = taskName;
    }

    protected abstract boolean canTake(
            AggregateItemStack sourceStock);

    protected abstract boolean villagerHasRequiredItems(Container villagerInventory);

    protected abstract int getItemCountRequiredForTask(
            AggregateItemStack sourceStock);
